            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">CREATE USERS WAREHOUSE ACCESS</h5>
                        <button type="button" class="close" data-dismiss="modal">
                            <span>&times;</span>
                        </button>
                    </div>
                    <form id="createForm">
                        <input type="number" hidden id="warehouseid" name="warehouseid" required>
                        <input type="number" hidden id="userid" name="userid" required>
                        <div class="modal-body">
                            <div class="form-column">
                                <label for="name" class="form-label mt-2"><strong>USER:</strong></label>
                                <select id="userSearch" required data-placeholder="Search User"></select>
                            </div>
                            <div class="form-column">
                                <label for="warehouseid" class="form-label mt-2">Warehouse</label>
                                <select id="warehouseSearch" required data-placeholder="Search Warehouse"></select>
                            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function fetchData() {
            ...
            $.ajax({
                url: "<?= base_url('ControllerCompany/getListUsersAccessWarehouse') ?>",
                method: "GET",
                dataType: "json",
                success: function (res) {
                    console.log(res);

                    res.listUsers.forEach((row, i) => {
                        dataFormatted.push([
                            i + 1,
                            row.username || '',
        ...
        $('#createForm').submit(function (e) {
            ...
            $.ajax({
                url: '<?= base_url('ControllerCompany/createUserWarehouseAccess') ?>',
                type: 'POST',
                data: formData,
                dataType: 'json',
        ...
        $("#userSearch").select2({
            width: '100%',
            ajax: {
                url: "ControllerMaster/searchUser",
                dataType: "json",
                delay: 250,
        ...
        $("#warehouseSearch").select2({
            width: '100%',
            ajax: {
                url: "ControllerMaster/searchWarehouse",
                dataType: "json",
                delay: 250,
                data: function (params) {
                    return { search: params.term };
        ...
        $("#warehouseSearch").on("select2:select", function (e) {
            let data = e.params.data;
        ...
        $('#createModal').on('hidden.bs.modal', function () {
            ...
            $('#warehouseSearch').val(null).trigger('change');
            $('#userid').val('');
            $('#warehouseid').val('');
        });
