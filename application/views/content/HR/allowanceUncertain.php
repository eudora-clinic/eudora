    ...
    <div class="card mb-4 shadow-sm">
        ...
        <div class="card shadow-sm">
            <div class="card-header d-flex justify-content-between align-items-center bg-white">
                <h5 class="mb-0">Data Tunjangan Tak Tentu</h5>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function simulateAjaxCall(month, location, showAlert) {
            ...
            $.ajax({
                url: '<?= base_url("ControllerHr/allowanceUncertain"); ?>',
                method: 'GET',
                data: filters,
                dataType: 'json',
    ...
    function addUncertainCompany() {
        ...
        $.ajax({
            url: '<?= base_url('ControllerHr/getAllowanceUncertain/') ?>0/' + month,
            type: 'GET',
            success: function (response) {
                // $('#uncertainAllowanceModal').remove();
                $('body').append(response);
                $('#uncertainAllowanceModal').modal('show');
            },
        });
    }
    ...
    function editUncertainCompany(id) {
        const month = $('#filterMonth').val();
        console.log('<?= base_url('ControllerHr/getAllowanceUncertain/') ?>' + id + '/' + month);


        $.ajax({
            url: '<?= base_url('ControllerHr/getAllowanceUncertain/') ?>' + id + '/' + month,
            type: 'GET',
            success: function (response) {
                $('#uncertainAllowanceModal').remove();
                $('body').append(response);
