...
<script>
    $(document).ready(function () {
        ...
        // Base URL untuk API
        const baseUrl = "<?= base_url('ControllerHr/employeePayroll'); ?>";
        ...
        function generatePayroll(showAlert = false) {
            const month = $('#filterMonth').val();
            const location = $('#filterLocation').val();
            ...
            generatePayrollFunction(month, location, showAlert);
        }
        function simulateAjaxCall(month, location, showAlert) {
            ...
            $.ajax({
                url: '<?= base_url("ControllerHr/generatePayrollPreview"); ?>',
                method: 'GET',
                data: filters,
                dataType: 'json',
                success: function (res) {
                    ...
                    if (res.status === 'success' && res.results.length == 0) setTimeout(() => {
                        ...
                        if (showAlert) toastr.success('Tidak ada data');
                        $('#summaryContainer').html(`
                        <div class="alert alert-info text-center">
                            <i class="fas fa-info-circle mr-2"></i>Tidak ada data untuk di generate bulan ini untuk perusahaan ini.
                        </div>
                    `);
                        return;
                    }, 1000);
        ...
        function generatePayrollFunction(month, location, showAlert) {
            const filters = {
                month: month,
                companyid: location
            }
            ...
            $.ajax({
                url: '<?= base_url("ControllerHr/generatePayroll"); ?>',
                method: 'GET',
                data: filters,
                dataType: 'json',
                success: function (res) {
