    ...
    <div class="hidden mt-2" id="role-information">
        ...
        <div class="card p-4">
            <div class="row center" style="gap: 30px">
                ...
                <div>
                    <button style="cursor: pointer" class="btn-sm btn-primary" id="btnAddTargetTest"
                        data-broadcastid="<?= $broadcastid ?>">ADD
                        TARGET
                        CUSTOMER TEST</button>
                    <div id="notifAddTargetTest" class="mt-2"></div>
                </div>

                <div>
                    <button style="cursor: pointer" class="btn-sm btn-primary" id="btnAddTargetSpesifik"
                        data-broadcastid="<?= $broadcastid ?>">ADD
                        SPESIFIK TARGET</button>
                    <div id="notifAddTargetSpesifik" class="mt-2"></div>
                </div>
...
<script>
    $(document).ready(function () {
        ...
        if (id) {
            $.ajax({
                url: `${BASE_URL}/ControllerApiApps/get_broadCastListById/${id}`,
                method: 'GET',
                dataType: 'json',
                success: function (res) {
                    if (res.status === 200 && res.data) {
                        ...
                        if (data.image_url) {
                            $('#image-preview').attr('src', `${BASE_URL}/${data.image_url}`).show();
                        } else {
                            $('#image-preview').hide();
                        }
                    } else {
            ...
            $('#tableTargetPromo').DataTable({
                ...
                serverSide: true,
                ajax: {
                     url: `${BASE_URL}/get_listTargetPromo/${id}`,
                    type: 'GET'
                },
                columns: [
...
<script>
    $(document).ready(function () {
        $('#btnAddTarget').on('click', function () {
            ...
            $.ajax({
                url: "<?= base_url('ControllerApiApps/insert_target_customer') ?>",
                method: 'POST',
        ...
        $('#btnAddTargetTest').on('click', function () {
            const broadcastId = $(this).data('broadcastid');
            ...
            $.ajax({
                url: "<?= base_url('ControllerApiApps/insert_target_customerTest') ?>",
                method: 'POST',
                data: { broadcast_id: broadcastId },
                dataType: 'json',
                beforeSend: function () {
                    $('#notifAddTargetTest').html('<span class="text-warning">Proses menyimpan...</span>');
                },
                success: function (res) {
                    if (res.status === 200) {
                        $('#notifAddTargetTest').html('<span class="text-success">' + res.message + '</span>');
                    } else {
                        $('#notifAddTargetTest').html('<span class="text-danger">' + res.message + '</span>');
                    }
                },
                error: function () {
                    $('#notifAddTargetTest').html('<span class="text-danger">Gagal menambahkan target customer</span>');
                }
            });
        });
    });
...
<script>
    $(document).ready(function () {
        $('#btnSendNotification').on('click', function () {
            ...
            $.ajax({
                url: "<?= base_url('ControllerApiApps/send_broadcast_notification_to_user') ?>",
                method: 'POST',
                data: { broadcast_id: broadcastId },
                dataType: 'json',
                beforeSend: function () {
...
<script>
    $(document).ready(function () {
        ...
        $('#btnUpdatePromo').on('click', function () {
            ...
            $.ajax({
                url: "<?= base_url('ControllerApiApps/updatePromoBroadcast') ?>",
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
