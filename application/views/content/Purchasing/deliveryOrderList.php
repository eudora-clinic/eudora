...
<div class="mycontaine container-fluid">
  <div class="card">
    ...
    <h3 class="card-header card-header-info d-flex justify-content-between align-items-center mt-3" style="font-weight: bold; color: #666;">
...
$(document).ready(function() {
  ...
  const tableApproved = $('#tableApproved').DataTable({
    columns: [
        ...
        render: function(d){
            ...
            return '<span class="badge bg-info">Requested to Stock In</span>';
            } else if (d == 2) {
      ...
      render: function(id, type, row){
        // tentukan link detail sesuai level
        let detailUrl = "";
        if (level == 22) {
          detailUrl = `https://sys.eudoraclinic.com:84/app/deliveryOrderChecked/${id}`;
        } else {
          detailUrl = `https://sys.eudoraclinic.com:84/app/deliveryOrder/${id}`;
        }

        // siapkan HTML dropdown
        return `
          <div class="btn-group">
            ...
            <ul class="dropdown-menu">
              <li>
                <a class="dropdown-item" href="${baseUrl}/ControllerPurchasing/generateDeliveryOrderPDF/${id}" target="_blank">
                  Cetak PDF
                </a>
              </li>
              <li>
  ...
  function loadData(date,company) {
    $.ajax({
      url: baseUrl + "/ControllerPurchasing/getAllDeliveryOrder",
      type: "GET",
      ...
      success: function(res){
        ...
        tableApproved.clear().rows.add(
            data.filter(x => [0,1,2].includes(Number(x.status)))
        ).draw();
  ...
  loadData($('#filterDate').val(),$('#filterCompany').val());

  // // Event: Detail
  // $(document).on('click', '.btn-detail', function(){
  //   const id = $(this).data('id');
  //   $.ajax({
  //     url: baseUrl + "/ControllerPurchasing/getPurchaseOrderById/" + id,
  //     type: "GET",
  //     dataType: "json",
  //     success: function(res) {
