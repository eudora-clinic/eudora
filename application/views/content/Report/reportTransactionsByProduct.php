            ...
            <div class="card p-2 col-md-12">
                ...
                <form id="form-cari-invoice">
                    <div class="row g-3" style="display: flex; align-items: center;">
                        ...
                        <div class="col-md-2">
                            <div class="form-group"> <label for="locationid">Lokasi</label>
                                <select name="locationid" id="locationid" class="form-control select2" required>
                                    <option value="">-- Pilih Lokasi --</option>
                                    <?php foreach ($locationList as $p): ?>
...
<script>
    ...
    $(document).ready(function () {
        $('#producttypeid').on('change', function () {
            ...
            $('#productid').empty().append('<option value="">-- Pilih Product --</option>');

            if (typeId) {
                $.ajax({
                    url: '<?= base_url("ControllerReport/getProductsByType/") ?>' + typeId,
                    type: "GET",
                    dataType: "json",
    ...
    $(document).ready(function () {
        ...
        function fetchData() {
            ...
            $.ajax({
                url: "<?= base_url('ControllerReport/getReportTransactionByProduct') ?>",
                method: "POST",
                data: formData,
                dataType: "json",
                success: function (res) {
                    ...
                    $('#reportTitle').text(`Report Transaksi: ${formatDate(start)} - ${formatDate(end)}`);
