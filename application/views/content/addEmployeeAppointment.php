...
<body>
    <div class="card p-4">
        <h4 class="text-secondary text-center">
            <i class="bi bi-wallet2"></i> ADD EMPLOYEE APPOINTMENT
        </h4>
        <form id="affiliateForm">
            ...
            <label for="locationIdSelectOrigin">LOCATION ORIGIN</label>
            <select id="locationIdSelectOrigin" class="form-control locationIdSelectOrigin" required>
            </select>
            ...
            <input type="number" name="originLocationId" id="originLocationId" hidden>
    ...
    <script>
        $(document).ready(function () {
            $("#affiliateForm").submit(function (e) {
                e.preventDefault();
                $.ajax({
                    url: "<?= base_url('App/addEmployeeAppt') ?>",
                    type: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
            ...
            $("#employeeId").select2({
                width: '100%',
                ajax: {
                    url: "App/searchEmployeeAppointment",
                    dataType: "json",
                    delay: 250,
            ...
            $("#locationIdSelectOrigin").select2({
                width: '100%',
                ajax: {
                    url: "ControllerMaster/searchLocationEmployeeAppointment",
                    dataType: "json",
            ...
            $("#locationIdSelectOrigin").on("select2:select", function (e) {
                let data = e.params.data;
                $("#originLocationId").val(data.id);
            });
