...
<body>
            ...
            <div class="card p-4">
                <div class="form-row">
                    <div class="form-column">
                        <label for="locationId" class="form-label mt-2"><strong>Location</strong><span class="text-danger">*</span></label>
                        <select id="locationId" name="locationId" class="" required="true" aria-required="true">
                            <option value="">-NONE-</option>
                            <?php foreach ($locationList as $j) { ?>
                                <option value="<?= $j['id'] ?>" <?= (isset($input['locationId']) && $input['locationId'] == $j['id'] ? 'selected' : '') ?>><?= $j['name'] ?></option>
                            <?php } ?>
                        </select>
                        <label for="initialStockDate" class="form-label mt-2"><strong>Cut Off Date</strong><span class="text-danger">*</span></label>
                        <input type="date" id="initialStockDate" name="initialStockDate">
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function addService() {
            ...
            let hasError = false;
            if (!name || !code || !isactive || !price || !section || !group) {
            ...
            $.ajax({
                url: "<?= base_url() . 'App/addInitialStock' ?>",
                type: 'POST',
                ...
                success: function(response) {
                    try {
                        alert('Berhasil menambah initial stock');
