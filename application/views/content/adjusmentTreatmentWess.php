...
<body>
            ...
            <div class="card p-4">
                <div class="form-row">
                    <div class="form-column">
                        <label for="invoiceLocationId" class="form-label mt-2"><strong>Invoice Location</strong><span class="text-danger">*</span></label>
                        <select id="invoiceLocationId" name="invoiceLocationId" class="" required="true" aria-required="true">
                            <option value="">-NONE-</option>
                            <?php foreach ($locationList as $j) { ?>
                                <option value="<?= $j['id'] ?>" <?= (isset($input['invoiceLocationId']) && $input['invoiceLocationId'] == $j['id'] ? 'selected' : '') ?>><?= $j['name'] ?></option>
                            <?php } ?>
                        </select>
                        <label for="customer-name" class="form-label mt-2"><strong>Customer Name</strong><span class="text-danger">*</span></label>
                        <select id="customer-name" class="customer-search" data-placeholder="Cari nama pasien"></select>
                        <input hidden type="number" id="customerId" name="customerId">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="items mt-2">
                ...
                <table id="tbl-items" class="table table-bordered items-list">
                    <thead class="bg-thead">
                        <tr>
                            <th style="font-size: 12px; text-align: center;">INVOICENO</th>
                            <th style="font-size: 12px; text-align: center;">TREATMENT</th>
                            <th style="font-size: 12px; text-align: center;">REMAIN</th>
                            <th style="font-size: 12px; text-align: center;">QTY TOTAL</th>
                            <th style="font-size: 12px; text-align: center;">AMOUNT</th>
                            <th style="font-size: 12px; text-align: center;">PURCHASE DATE</th>
    ...
    <script>
                ...
                const itemHtml = `
                                        <tr>
                                            <td style="text-align: center;">
                                                <input type="text" class="form-control invoiceno">
                                            </td>
                                            <td style="text-align: center;">
                                            ...
                                            <td style="text-align: center;">
                                                <input type="number" class="form-control qtytotal" value="1" min="1">
                                            </td>
                                            <td style="text-align: center;">
        ...
        function addPackage() {
            const invoiceLocationId = document.getElementById('invoiceLocationId').value;
            const customerId = document.getElementById('customerId').value;

            let hasError = false;
            if (!invoiceLocationId || !customerId) {
            ...
            document.querySelectorAll('.items-list tbody tr').forEach(method => {
                ...
                const qtytotal = method.querySelector('.qtytotal').value;
                const invoiceno = method.querySelector('.invoiceno').value;
                const amount = method.querySelector('.amount').value;
                const purchasedate = method.querySelector('.purchasedate').value;

                if (!itemid || !qty || !qtytotal || !invoiceno || !amount || !purchasedate) {
            ...
            $.ajax({
                url: "<?= base_url() . 'App/addAdjusmentTreatmentWess' ?>",
                type: 'POST',
