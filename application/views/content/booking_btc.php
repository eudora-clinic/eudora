...
$split_emp              = $split_values[4];

$sql = "
        SELECT a.id, a.name, b.locationid, c.id, c.name as jobname FROM msemployee a
...
$sp_absen_doing = $db_oriskin->query("SELECT LOCATIONID, LOCATIONNAME, ABSENCEDATE, TIMESTART, TIMEFINISH, EMPLOYEEID, EMPLOYEENAME FROM [fnReportAbsenceDoingEudora]('$split_period', $split_locationid) where EMPLOYEEID = '$split_employeeid' and TIMESTART = '$split_timestart' ")->row();
...
$frontdesk = $db_oriskin->query("
        ...
        AND (c.name like '%FRONT DESK%' or c.name like '%BEAUTY THERAPIST%' or c.name like '%DOKTER%' or c.name LIKE '%CONSULTANT%' )
        ORDER BY a.name
    ")->result_array();

$count_type_new = $db_oriskin->query("
            SELECT count(*) as jml_new FROM [fnReportAbsenceDoingEudora]('" . $split_period . "', $split_locationid) as a
            join [fnReportSlotByType]('" . $split_period . "', $split_locationid) as b on a.TIMESTART=b.TIMESTART
            WHERE a.TIMESTART = '" . $split_timestart . "'
            AND a.MEMBERTYPE = 'NEW'
            AND a.ABSENCEDATE = '" . $split_period . "'
        ")->row();

$count_type_lc = $db_oriskin->query("
            SELECT count(*) as jml_lc FROM [fnReportAbsenceDoingEudora]('" . $split_period . "', $split_locationid) as a
            join [fnReportSlotByType]('" . $split_period . "', $split_locationid) as b on a.TIMESTART=b.TIMESTART
            WHERE a.TIMESTART = '" . $split_timestart . "'
            AND a.MEMBERTYPE = 'LC'
            AND a.ABSENCEDATE = '" . $split_period . "'
...
$data_slot = $db_oriskin->query("
            SELECT * FROM [fnReportSlotByType]('" . $split_period . "', $split_locationid)
            WHERE TIMESTART = '" . $split_timestart . "'
        ")->row();

$data_treatmentdate_trdoingtreatment = $db_oriskin->query("
            select treatmentdate from trdoingtreatment
            where treatmentdate = '" . $split_period . "'
            and starttreatment = '" . $split_timestart . "'
            and locationid = '" . $split_locationid . "'
        ")->row();
    ...
    <div class="mycontaine">
        <div class="row">
            <div class="card p-2 col-md-6">
                <form action="<?= base_url('App/search_data_booking_btc') ?>" method="post">
                                        ...
                                        <?php foreach ($data_member as $row) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                        ...
                                                        <span class="btn-primary btn" style="cursor: pointer;" data-toggle="modal" data-target="#view<?= $row['ID'] ?>">View</span>
...
?>
    ...
    <?php foreach ($data_member as $row) {
        ...
        //TAMBAHAN UNTUK PROMO MARKETING TIDAK BISA BOOKING DI JAM LAIN
        $doingData = $db_oriskin->query("SELECT ISNULL(a.invoiceno, '') AS invoiceno from trdoingtreatment a inner join slinvoicemembershiphdr b on a.invoiceno = b.invoiceno 
                                            inner join slinvoicemembershipdtl c on c.id = b.id 
                                            inner join msproductmembershiphdr d on d.id = c.productmembershiphdrid  where a.customerid = '" . $row['IDFROMDB'] . "' 
                                            and a.status = 2 and d.name like '%PROMO MARKETING%' ")->row()->invoiceno ?? 'NO';
        //TAMBAHAN UNTUK PROMO MARKETING TIDAK BISA BOOKING DI JAM LAIN
    ?>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane show active" id="treatment-info<?= $row['ID'] ?>">
                                        <div class="table-responsive">
                                            <table id="tbl-treatment-info<?= $row['ID'] ?>" class="table table-striped table-bordered" style="width:100%">
                                                <thead class="bg-thead">
                                                    <tr role="">
                                                    ...
                                                    if (count($treatment_info) > 0) {
                                                        ...
                                                        foreach ($treatment_info as $ti) { {
                                                                ...
                                                                if ($ti['STATUS'] == 2) {
                                                                    if ($ti['REMAINING'] > 0) {
                                                                        //$status = 'Ready';
                                                                        $status = '<button class="btn btn-sm btn-primary btn-xs check-in-treatment" data-toggle="modal" data-target="#modalNewDoingTreatment" onclick="initModalNewDoingTreatment(this,\'' . $ti['INVOICENO'] . '\',\'' . $ti['TREATMENTID'] . '\',\'' . $ti['TREATMENTNAME'] . '\',\'' . $row['IDFROMDB'] . '\',\'' . $split_emp . '\');">Booking</button>';
                                                                        $bgcolor = 'bg-success';
                                                                    } else {
                                                                        $status = 'Completed';
                                                                        $bgcolor = 'bg-danger';
                                    ...
                                    <div class="tab-pane" id="membership-treatment<?= $row['ID'] ?>">
                                        <div class="table-responsive">
                                            <table id="tbl-membership-treatment<?= $row['ID'] ?>" class="table table-striped table-bordered table-responsive" style="width:100%">
                                                <thead class="bg-thead">
                                                    <tr role="">
                                                    <th style="text-align: center; font-size: 12px !important;">Invoice #</th>
                                                    ...
                                                    if (count($membership_treatment) > 0) {
                                                        foreach ($membership_treatment as $mt) {
                                                            ...
                                                            if ($mt['STATUS'] == 2 && $mt['ISFULL'] == 0) {
                                                                if ($mt['REMAINING'] > 0) {
                                                                    //$status = 'Ready';
                                                                    $status = '<button class="btn btn-sm btn-primary btn-xs check-in-treatment" data-toggle="modal" data-target="#modalNewDoingTreatment" onclick="initModalNewDoingTreatment(this,\'' . $mt['INVOICENO'] . '\',\'' . $mt['TREATMENTID'] . '\',\'' . $mt['TREATMENTNAME'] . '\',\'' . $row['IDFROMDB'] . '\',\'' . $split_emp . '\');">Booking</button>';
                                                                    $bgcolor = 'bg-success';
                                                                } else {
                                                                    $status = 'Completed';
                                                                    $bgcolor = 'bg-danger';
                                                            ...
                                                            //TAMBAHAN UNTUK PROMO MARKETING TIDAK BISA BOOKING DI JAM LAIN
                                                            if ($doingData != 'NO') {
                                                                if ($mt['INVOICENO'] == $doingData && $mt['TREATMENTNAME'] != 'DOCTER CONSULTATION' && $mt['TREATMENTNAME'] != 'CRYOLIPOLYSIS PER 2 CUPS') {
                                                                    $status = 'already used';
                                                                    $bgcolor = 'bg-info';
                                                                }
                                                            }
        ...
        <div class="modal-dialog modal-lg" role="dialog" style="max-height: 500px; margin: auto; ">
            <div class="modal-content">
                <form class="form-horizontal" action="<?= base_url('savedoingtreatment') ?>" role="form" method="post">
                    <input type="hidden" id="idfromdb" name="idfromdb" class="form-control form-control-sm pl-2" required readonly />
                    <input type="hidden" id="empid" name="empid" class="form-control form-control-sm pl-2" required readonly />
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                        <h5 class="modal-title" id="modalNewDoingTreatmentLabel">ADD DOING TREATMENT</h5>
                    </div>
                    <div class="modal-body">
                        <div class="row mb-4">
                            <div class="col-md-4">
...
<script>
    ...
    function initModalNewDoingTreatment(_obj, _invoiceno, _treatmentid, _treatmentname, _idfromdb, _empid) {
        ...
        $(document).find('#empid').val(_empid);
...
<script>
    $('#existingcustomer').removeClass('display').addClass(
     ...
     $('#tbl-treatment-info<?= $row['ID'] ?>').removeClass('display').addClass(
        'table table-striped table-hover table-compact');
</script>