...
<?php
    ...
    // echo 'employid: '. $split_employeeid. '<br>';
    // echo 'employid: '. $split_timestart. '<br>';
    // echo 'employid: '. $split_period. '<br>';
    // echo 'employid: '. $split_locationid. '<br>';

    $sp_absen_doing = $db_oriskin->query("SELECT LOCATIONID, LOCATIONNAME, ABSENCEDATE, TIMESTART, TIMEFINISH, EMPLOYEEID, EMPLOYEENAME FROM [fnReportAbsenceDoing]('$split_period', $split_locationid) where EMPLOYEEID = '$split_employeeid' and TIMESTART = '$split_timestart' ")->row();
    ...
    // PEMBATASAN
        $count_type_new = $db_oriskin->query("
            SELECT count(*) as jml_new FROM [fnReportAbsenceDoing]('".$split_period."', $split_locationid) as a
        ...
        $count_type_lc = $db_oriskin->query("
            SELECT count(*) as jml_lc FROM [fnReportAbsenceDoing]('".$split_period."', $split_locationid) as a
            join [fnReportSlotByType]('".$split_period."', $split_locationid) as b on a.TIMESTART=b.TIMESTART
            WHERE a.TIMESTART = '".$split_timestart."'
            AND a.MEMBERTYPE = 'LC'
            AND a.ABSENCEDATE = '".$split_period."'
        ...
        // echo 'count_type_new: '.$count_type_new->jml_new.'<br><br>';
        // echo 'count_type_lc: '.$count_type_lc->jml_lc.'<br><br>';
        // echo 'SLOT NEW: '.$data_slot->SLOTNEW.'<br><br>';
        // echo 'SLOT LC: '.$data_slot->SLOTLC.'<br><br>';
    // END PEMBATASAN
?>
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header card-header-info">
            		<h4 class="card-title">Search Existing Member</h4>
        		</div>

                <div class="card-body">
                    <form action="<?= base_url('App/search_data_booking_btc') ?>" method="post">
                        <div class="row">
                            <!-- <label for="">Name / HP / SSID / Customer Code / Member Code :</label> -->
                            <div class="col-md-6">
                                <label>Select Name:</label>
                                <input type="hidden" name="etpl" value="<?= $etpl ?>">
                                <input type="text" class="form-control" name="search" value="<?= set_value('search') ?>" required>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header card-header-info">
            		<h4 class="card-title">ERM Booking</h4>
        		</div>

                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered text-center" id="table1" style="width: 100% !important;">
                            <thead>
                                <tr class="bg-info text-white width-th">
                                ...
                                <tbody>
                                    <?php foreach($data_member as $row){?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if(@$data_treatmentdate_trdoingtreatment->treatmentdate != null){ ?>
                                                    <?php
                                                    $get_date = $data_treatmentdate_trdoingtreatment->treatmentdate;

                                                    $current_date = date('Y-m-d');

						                            $new_date = date('Y-m-d', strtotime($current_date . ' +3 days'));

                                                    if(strtotime($new_date) <= strtotime($get_date)){ ?>
                                                        <!-- Sudah lewat 3 hari -->
                                                        <?php if($row['TYPECUSTOMER'] == 'NEW'){ ?>
                                                            <?php if($count_type_new->jml_new < $data_slot->SLOTNEW){ ?>
                                                                <span class="badge badge-info" style="cursor: pointer;" data-toggle="modal" data-target="#view<?= $row['ID'] ?>">View</span>
                                                            <?php } ?>
                                                        <?php }elseif($row['TYPECUSTOMER'] == 'LC'){ ?>
                                                            <?php if($count_type_lc->jml_lc < $data_slot->SLOTLC){ ?>
                                                                <span class="badge badge-info" style="cursor: pointer;" data-toggle="modal" data-target="#view<?= $row['ID'] ?>">View</span>
                                                            <?php } ?>
                                                        <?php } ?>
                                                    <?php }else{ ?>
                                                ...
                                                <div class="table-responsive">
                                                    <table id="tbl-treatment-info<?= $row['ID'] ?>" class="table table-bordered display highlight cell-border row-border table-responsive" style="width:100%">
                                                        <thead class="thead-danger">
                                                            <tr role="" class="bg-info text-white width-th">
                ...
                <div class="modal-dialog modal-lg" role="dialog">
                    <div class="modal-content">
                        <form class="form-horizontal" action="<?= base_url('savedoingtreatment') ?>" role="form" method="post">
                            ...
                            <input type="hidden" id="idfromdb" name="customerid" class="form-control form-control-sm pl-2" required readonly/>
	...
	$(document).ready(function () {
		...
		$(".btn-save").each(function(index) {
			$(this).on("click", function(){
				...
				$.post("<?= base_url('save-note-history-doing') ?>", {doingid: id, note: note}, function(res) {
					//
				}).done(function(res) {
					console.log(res);
				}).fail(function() {
