...
$db_oriskin = $this->load->database('oriskin', true);
$query = $db_oriskin->query("EXEC [spReportCallApptDetailPotentialLeaver]  '" . $currentMonth . "','" . $locationid . "' ");
							...
							<tbody>
								...
								if (isset($header)) {
									foreach ($header as $v) {
										...
										$note = $db_oriskin->query("SELECT ISNULL(remarks, '') AS note FROM slpotentialleaver_note WHERE CUSTOMERID = '" . $v['CUSTOMERID'] . "'")->row()->note ?? '';
										$calldate = $db_oriskin->query("SELECT ISNULL(calldate, '') AS calldate FROM slpotentialleaver_note WHERE CUSTOMERID = '" . $v['CUSTOMERID'] . "'")->row()->calldate ?? '';
	...
	$(document).ready(function() {
		...
		$(".btn-save").each(function(index) {
			$(this).on("click", function() {
				...
				$.post("<?= base_url('save-note-followup-doing-potential-leaver') ?>", {
					customerid: id,
					note: note,
					period: period,
