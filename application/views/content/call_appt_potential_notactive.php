...
$db_oriskin = $this->load->database('oriskin', true);
$query = $db_oriskin->query("EXEC [spReportCallApptDetailPotentialNotActive]  '" . $currentMonth . "','" . $locationid . "' ");
		...
		<div class="col-md-12">
			...
			<div class="card">
				<div class="card-header card-header-primary">
					<h4 class="card-title">APPT POTENTIAL NOT ACTIVE</h4>
				</div>
				<div class="card-body">
							...
							<tbody>
								...
								if (isset($header)) {
									foreach ($header as $v) {
										...
										$note = $db_oriskin->query("SELECT ISNULL(remarks, '') AS note FROM slpotentialnotactive_note WHERE CUSTOMERID = '" . $v['CUSTOMERID'] . "'")->row()->note ?? '';
										$calldate = $db_oriskin->query("SELECT ISNULL(calldate, '') AS calldate FROM slpotentialnotactive_note WHERE CUSTOMERID = '" . $v['CUSTOMERID'] . "'")->row()->calldate ?? '';
	...
	$(document).ready(function() {
		...
		$(".btn-save").each(function(index) {
			$(this).on("click", function() {
				...
				$.post("<?= base_url('save-note-followup-doing-potential-notactive') ?>", {
					customerid: id,
					note: note,
					period: period,
