...
<script>
    ...
    function sendImageMessage() {
        ...
        $.ajax({
            url: "https://sys.eudoraclinic.com:84/apieudora/sendMessagesImagesByEmployee",
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
<script>
    ...
    async function loadChatList() {
        try {
            const response = await fetch('https://sys.eudoraclinic.com:84/apieudora/getChatList?user_id=10&user_type=employee');
    ...
    function loadChatDetail(receiver_id, receiver_type) {
        fetch(`https://sys.eudoraclinic.com:84/apieudora/detailConsultationChat?receiver_id=${receiver_id}&receiver_type=${receiver_type}`)
            ...
            .then(res => {
                ...
                if (res.status === 'success') {
                    res.data.messages.forEach(msg => {
                        const isUser = msg.sender_id == 10 && msg.sender_type == 'employee';
                        ...
                        if (msg.type === 'image' && msg.message) {
                            ...
                            <div class="bubble ${bubble}" style="cursor: pointer;">
                                <img src="https://sys.eudoraclinic.com:84/apieudora/${msg.message}" alt="Image message" style="max-width: 200px; border-radius: 10px;" />
                            </div>
    ...
    function setupWebSocket() {
        ...
        socket.on("connect", () => {
            ...
            // Join ke room user saat connect
            socket.emit("joinRoom", "employee_10");
        });

        socket.on("newMessage", (data) => {
            console.log("📥 Message received:", data);
            if (data.receiver_id == 10 && data.receiver_type == "employee") {
                loadChatList();
                console.log(activeReceiverId, activeReceiverType);
...
<script>
    document.getElementById('message-form').addEventListener('submit', function (e) {
        ...
        $.ajax({
            url: "https://sys.eudoraclinic.com:84/apieudora/sendMessagesEmployee",
            type: 'POST',
            data: formData,
            dataType: 'json',
            success: function (data) {
