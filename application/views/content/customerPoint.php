    	...
    	<div class="col-md-12">
      		<div class="card">
        		<div class="card-header card-header-danger">
            		<h4 class="card-title">Data Customer Point</h4>
        		</div>
				<div class="card-body">
					...
					<div>
						<?php
							...
							# query
						$query = $db_oriskin->query("select a.id as Customerid,a.id,A.firstname + (CASE WHEN A.lastname IS NULL THEN '' ELSE ' ' + A.lastname END) AS Lcname,a.cellphonenumber as Cellphone,(b.pointTr+b.pointMgm-b.pointAdjust+isnull(c.point,0)) as Point
                            from mscustomer a inner join msPoint b on a.id = b.customerid
							     left join slpointadjust c on a.id=c.customerid
                            where a.locationid='".$clinicid."'");
						...
						<table id="example" class="table table-bordered" style="width:100%">
									<thead class="thead-danger">
                                        ...
                                        <th width="30px" class="text-center">No</th>
                                        						<th width="90px" class="text-center">Customer Id</th>
											<th width="170px" class="first-col sticky-col">LC Name</th>
											<th width="120px" class="text-center">Cellphone</th>
											<th width="90px" class="text-center">Point</th>
                                            <th width="350px" class="text-center">Follow Up</th>
										</tr>
									</thead>
									<tbody>
										<?php
											if (isset($header)) {
												...
												foreach ($header as $v)
													{
                                                        $note = $db_oriskin->query("SELECT ISNULL(note, '') AS note FROM historyfollowupcustomerpoint WHERE CUSTOMERID = '".$v['Customerid']."'")->row()->note ?? '';
													echo '<tr>
                                                <td class="text-center">'.++$no.'</td>
												<td class="first-col sticky-col">'.$v['Customerid'].'</td>
												<td class="first-col sticky-col">'.$v['Lcname'].'</td>
												<td class="text-left">'.$v['Cellphone'].'</td>
												<td class="text-center">'.number_format($v['Point'], 0, ',', '.').'</td>
	...
	$(document).ready(function () {
		...
		$(".btn-save").each(function(index) {
			$(this).on("click", function(){
				...
				$.post("<?= base_url('save-note-followup-customer-point') ?>", {customerid: id, note: note}, function(res) {
					//
				}).done(function(res) {
					console.log(res);
				}).fail(function() {
