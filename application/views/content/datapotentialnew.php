    	...
    	<div class="col-md-12">
      		<div class="card">
        		<div class="card-header card-header-danger">
            		<h4 class="card-title">Data New Leaver</h4>
        		</div>
				<div class="card-body">
				<div class="toolbar">
					</div>
					...
					<div>
						<?php
							...
							# query
							$query = $db_oriskin->query("EXEC spReportPotentialNewLC '2023-06', '".$no_invoice."' ");
						...
						<table id="example" class="table table-bordered" style="width:100%">
									<thead class="thead-danger">
										...
										<th width="90px" class="text-center">Id</th>
											<th width="180px" class="text-center">Name</th>
											<th width="190px" class="text-center">Cellphone</th>
											<th width="110px" class="text-center">L.Transaction</th>
											<th width="250px" class="text-center">L.Purchase</th>
											<th width="150px" class="text-center">L.Doing</th>
									...
									<tbody>
										<?php
											if (isset($header)) {
												...
												foreach ($header as $v)
													 {
														...
														$note = $db_oriskin->query("SELECT ISNULL(note, '') AS note FROM historyfollowuppotentialnewlc WHERE CUSTOMERID = '".$v['CUSTOMERID']."'")->row()->note ?? '';
												...
												<td class="first-col sticky-col">'.$v['LASTRANSACTION'].'</td>
												<td class="text-center">'.$v['LASTPURCHASE'].'</td>
	...
	$(document).ready(function () {
		...
		$(".btn-save").each(function(index) {
			$(this).on("click", function(){
				...
				$.post("<?= base_url('save-note-followup-potentialnewlc') ?>", {customerid: id, note: note}, function(res) {
					//
				}).done(function(res) {
					console.log(res);
				}).fail(function() {
