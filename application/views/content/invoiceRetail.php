...
<body>
    <div class="mycontaine">
                ...
                <div class="col-md-9">
                    <div class="form-group">
                        <input type="text" id="no-invoice" name="no-invoice" class="form-control" required="true"
                            aria-required="true" placeholder="INVOICE RETAIL">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
        ...
        <div class="hidden">
            ...
            <button id="btnStatusInvoiceActive" type="button" class="btn btn-sm btn-primary"
                onclick="updateStatusInvoice(2)" style="background-color: #c49e8f; color: black;">ACTIVE</button>
                ...
                <div class="card p-4">
                    <div class="form-row">
                        <div class="form-column">
                            ...
                            <input hidden type="number" name="invoicehdrid" id="invoicehdrid">

                            <label for="invoiceno" class="form-label mt-2"><strong>INVOICE:</strong><span
                                    class="text-danger">*</span></label>
                        ...
                        <div class="form-column">
                            <label for="invoicedate" class="form-label mt-2"><strong>PURCHASEDATE:</strong></label>
    ...
    <div class="modal fade" id="updateModalProduct" tabindex="-1" role="dialog" aria-labelledby="updateModalProductLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="updateProductForm">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                <input type="number" name="amountupdateProduct" id="amountupdateProduct" class="form-control"
                                    style="border: 1px solid #ced4da; border-radius: 6px; padding: 10px 12px; font-size: 16px; box-shadow: inset 0 1px 2px rgba(0, 0, 0, 0.1); transition: border-color 0.3s, box-shadow 0.3s;"
                                    placeholder="Amount">
                            </div>
                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#tbl-payment tbody').on('click', '.delete-btn', function() {
            ...
            if (confirm("Yakin menghapus payment method?")) {
                $.ajax({
                    url: "<?= base_url('App/deletePaymentMethod'); ?>", // Sesuaikan dengan URL
                    type: "POST",
                    data: {
                        id: invoicepaymentid,
                        type: 3
        ...
        $("#product-name-update").on("select2:select", function(e) {
         ...
         $('#updateProductForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "<?= base_url('App/updateProductInvoice'); ?>",
                type: "POST",
                data: formData,
                dataType: "json", // pastikan ini "json" supaya response otomatis diparse
                success: function(response) {
    ...
    function searchInvoice() {
        ...
        $.ajax({
            url: "<?= base_url('App/search_invoice') ?>", // <-- Ganti sesuai controller dan method kamu
            method: 'GET',
            data: data,
            dataType: 'json',
            beforeSend: function() {
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $('#invoicehdrid').val(response.data.invoicehdrid);

                    table.clear();
                    let dataSet = [];
                    response.dataPayment.forEach(function(row) {

                        editBtn = `<button class="btn btn-primary btn-sm update-btn text-center" data-payment-amount="${row.AMOUNT}" data-id="${row.INVOICEPAYMENTID}" data-payment-id="${row.PAYMENTID}">EDIT</button>`;
                        deleteBtn = `<button class="btn btn-danger btn-sm delete-btn text-center" data-id="${row.INVOICEPAYMENTID}">DELETE</button>`;
    ...
    function addPaymentMethod() {
        const invoicehdrid = document.getElementById('invoicehdrid').value;
        let hasError = false;
        if (!invoicehdrid) {
        ...
        if (confirm("Yakin menambah metode?!")) {
            $.ajax({
                url: "<?= base_url() . 'App/saveAddPaymentMethod' ?>",
                type: 'POST',
    ...
    function updateStatusInvoice(status) {
        ...
        if (confirm("Yakin menambah metode?!")) {
            $.ajax({
                url: "<?= base_url() . 'App/updateStatusInvoice' ?>",
                type: 'POST',
                data: JSON.stringify(transactionData),
                contentType: 'application/json',
                dataType: 'json',
    ...
    function updateInvoice() {
        ...
        let hasError = false;
        if (!invoicehdrid || !customerid || !invoiceno || !salesid || !invoicedate) {
            alert('Terjadi kesalahan silahkan di refresh atau login ulang!');
            return false;
        }
        ...
        if (confirm("Yakin update invoice?!")) {
            $.ajax({
                url: "<?= base_url() . 'App/updateInvoice' ?>",
                type: 'POST',
                data: JSON.stringify(transactionData),
                contentType: 'application/json',
                dataType: 'json',
