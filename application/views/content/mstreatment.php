    	...
    	<div class="col-md-12">
      		<div class="card">
        		<div class="card-header card-header-success">
            		<h4 class="card-title">Update List Treatment</h4>
        		</div>
				<div class="card-body">
					<div class="toolbar">
						<form id="form-cari-invoice" method="get" action="<?= current_url() ?>">
							<div class="row">
								<label class="col-md-2 col-form-label">Treatment Name</label>
								<div class="col-md-4">
									<div class="form-group bmd-form-group">
										<input type="text" id="treatmentname" name="treatmentname" class="form-control" required="true" aria-required="true" placeholder="ketik name Treatment yang akan dicari" value="<?= (isset($_GET['treatmentname']) ? $_GET['treatmentname'] : '') ?>">
									</div>
								</div>
					...
					<div id="result" style="display: <?= (isset($_GET['submit']) ? 'block;' : 'none;') ?>">
						<?php
							...
							if (isset($_GET['submit'])) {
								$treatmentname = $this->input->get('treatmentname');
								# load database oriskin (lihat di config/database.php)
								$db_oriskin = $this->load->database('oriskin', true);
								# query
								$query = $db_oriskin->query("select TOP (100)  id,name,price,isactive from mstreatment where name like '%".$treatmentname."%' order by name");
								if ($query->num_rows() <= 0) {
									echo '<div class="alert alert-danger">
												<button type="button" class="close" data-dismiss="alert" aria-label="Close">
													<i class="material-icons">close</i>
												</button>
												<span>Treatment Name <strong>'.$treatmentname.'</strong> tidak ditemukan</span>
											</div>';
								} else { 
								...
								<table id="dt-invoice-membership" class="table table-bordered" cellspacing="0" width="100%" role="grid">
									<thead>
										<tr role="">
											...
											<th width="20%" class="second-col sticky-col"><b>PRICE</b></th>
											...
											if (isset($header)) {
												foreach ($header as $v) {
													echo '<tr>
															...
															<td class="text-center">
																<span id="sp-price-'.$v['id'].'">'.$v['price'].'</span>
																<input type="text" id="price-'.$v['id'].'" value="'.$v['price'].'" style="width: 100%; display: none;">
															</td>
															<td class="text-center">
...
<script>
	...
	function editHeader(_id, e) {
		...
		$('#price-'+_id).val($('#sp-price-'+_id).html());
	...
	function saveHeader(_id, e) {
		...
		if (_confirm) {
			...
			$.post(_HOST+'App/updateMsTreatment', {id:_id,price:_price,isactive:_isactive}, function(result) {
				alert(result);
				location.reload();
			});
		} else {
