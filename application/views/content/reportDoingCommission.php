        ...
        <div class="mycontaine">
            <?php if ($level == 1) { ?>
                <div class="card p-2 col-md-6">
                    <form id="form-cari-invoice" method="post" action="<?= base_url('reportDoingCommission') ?>">
                        <div class="row g-3">
                            <div class="col-md-4">
                    ...
                    <form id="form-cari-invoice" method="post" action="<?= base_url('reportDoingCommission') ?>">
                        <div class="row g-3">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="">Date Start</label>
                                    <input id="dateStart" type="date" name="dateStart" class="form-control filter_period"
                                        value="<?= $dateStart ?>" required>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="">Date End</label>
                                    <input id="dateEnd" type="date" name="dateEnd" class="form-control filter_period"
                                        value="<?= $dateEnd ?>" required>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <select id="locationId" name="locationId" class="form-control text-uppercase "
                                        ...
                                        <?php foreach ($locationList as $j) { ?>
                                            <option value="<?= $j['id'] ?>" <?= ($outletId == $j['id'] ? 'selected' : '') ?>>
                                                <?= $j['name'] ?>
                                            </option>
                                        <?php } ?>
                                    </select>
                    ...
                    <div class="col-md-12 mt-3">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($listHandWorkDokterTherapist as $row) {
                                                    ...
                                                    <tr role="" style="font-weight: 400;">
                                                        ...
                                                        <td style="text-align: center;"><?= $row['QTYTREATMENT'] ?></td>
                                                        ...
                                                        <td style="text-align: right;">
                                                            <?= number_format($row['COMMISSIONDOING'], 0, ',', ',') ?>
                                                        </td>
                                                        <td style="text-align: right;">
                        ...
                        <div class="panel-body tab-pane show active" id="hwTherapist">
                            <div class="card">
                                ...
                                <?php if ($level == 4): ?>
                                    <button class="btn btn-sm btn-primary" id="btnGenerate">Generate To Payroll
                                        Commission</button>
                                <?php endif; ?>
                                        ...
                                        <table id="tableHwTherapist" class="table table-striped table-bordered"
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($summaryHandWork as $row) {
                                                    $rowStyle = ($row['isgenerate'] !== null && $row['isgenerate'] !== "")
                                                        ? 'style="background-color: #d4edda;"'  // hijau muda bootstrap
                                                        : '';
                                                    ?>
                                                    <tr <?= $rowStyle ?>>
                                                        <td style="text-align: center;"><?= $no++ ?></td>
    ...
    $(document).ready(function () {
        ...
        var table = $('#tableDailySales').DataTable({
            ...
            "footerCallback": function (row, data, start, end, display) {
                ...
                var sumCol11Filtered7 = display.map(el => data[el][11]).reduce((a, b) => intVal(a) + intVal(b), 0);
                var sumCol11Filtered8 = display.map(el => data[el][10]).reduce((a, b) => intVal(a) + intVal(b), 0);
                var sumCol11Filtered11 = display.map(el => data[el][15]).reduce((a, b) => intVal(a) + intVal(b), 0);
        ...
        $('#filterSection').on('change', function () {
            table.column(7).search(this.value).draw(); // Kolom ke-8 (Branch)
        });
        ...
        $('#filterStaff').on('change', function () {
            ...
            table.column(6).search(valuesss, true, false).draw(); // Kolom ke-8 (Branch)
        });


        $('#btnGenerate').on('click', function () {
            let confirmActive = confirm("Yakin ingin generate data payroll periode ini di perusaan ini?");
            if (confirmActive) {
                generatePayrollCommission(true);
            }
        });

        function generatePayrollCommission(showAlert = false) {
            const dateStart = $('#dateStart').val();
            const dateEnd = $('#dateEnd').val();
            const locationId = $('#locationId').val();

            if (!dateStart || !dateEnd || !locationId) {
                toastr.warning('Pilih tangga dan lokasi terlebih dahulu!');
                return;
            }

            generatePayrollFunction(dateStart, dateEnd, locationId, showAlert);
        }


        function generatePayrollFunction(dateStart, dateEnd, locationId, showAlert) {
            const filters = {
                dateStart: dateStart,
                dateEnd: dateEnd,
            ...
            $.ajax({
                url: '<?= base_url("ControllerCommission/generatePayrollCommissionHandWork"); ?>',
                method: 'GET',
                data: filters,
                dataType: 'json',
                success: function (res) {
                    if (res.status === 'success' && res.results.length > 0) setTimeout(() => {
                        if (showAlert) toastr.success('Data berhasil digenerate');
                        setTimeout(() => {
                            location.reload();
                        }, 800)
                    }, 1000);
                    ...
                    if (res.status === 'success' && res.results.length == 0) setTimeout(() => {
                        if (showAlert) toastr.success('Tidak ada data untuk digenerate');
                    }, 1000);
                },
                error: function (xhr) {
