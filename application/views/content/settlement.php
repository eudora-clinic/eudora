				...
				<div class="card-body">
					<div class="toolbar">
						<form id="form-cari-invoice" method="get" action="<?= current_url() ?>">
							<div class="row">
								<label class="col-md-2 col-form-label">Location ID</label>
								<div class="col-md-4">
									<div class="form-group bmd-form-group">
										<input type="text" id="locationid" name="locationid" class="form-control" required="true" aria-required="true" placeholder="ketik no invoice yang akan dicari" value="<?= (isset($_GET['locationid']) ? $_GET['locationid'] : '') ?>">
									</div>
								</div>
					...
					<div id="result" style="display: <?= (isset($_GET['submit']) ? 'block;' : 'none;') ?>">
						<?php
							...
							if (isset($_GET['submit'])) {
								...
								# query
								$query = $db_oriskin->query("select  id, locationid, settlementdate, status from trbalancingsettlementhdr where locationid = '".$locationid."' order by settlementdate DESC");
								if ($query->num_rows() <= 0) {
									echo '<div class="alert alert-danger">
												<button type="button" class="close" data-dismiss="alert" aria-label="Close">
													<i class="material-icons">close</i>
												</button>
												<span>No. Invoice <strong>'.$locationid.'</strong> tidak ditemukan</span>
											</div>';
								} else { 
								...
								<table id="dt-invoice-membership" class="table table-bordered" cellspacing="0" width="100%" role="grid">
									<thead>
										<tr role="">
											...
											<th width="20%" class="second-col sticky-col"><b>LOCATION ID</b></th>
											<th width="20%" class="second-col sticky-col"><b>SETTLEMENT DATE</b></th>
												...
												foreach ($header as $v) {
													echo '<tr>
															...
															<td class="text-center">
																...
																<input type="text" id="locationid-'.$v['id'].'" value="'.$v['locationid'].'" style="width: 100%; display: none;">
															</td>
															<td class="text-center">
																<span id="sp-settlementdate-'.$v['id'].'">'.$v['settlementdate'].'</span>
																<input type="text" id="settlementdate-'.$v['id'].'" value="'.$v['settlementdate'].'" style="width: 100%; display: none;">
															</td>
															<td class="text-center">
...
<script>
	...
	function editHeader(_id, e) {
		...
		$('#sp-settlementdate-'+_id).hide();
		$('#sp-status-'+_id).hide();
		$('#btn-edit-header-'+_id).hide();
		$('#id-'+_id).val($('#sp-id-'+_id).html());
		$('#locationid-'+_id).val($('#sp-locationid-'+_id).html());
		$('#settlementdate-'+_id).val($('#sp-settlementdate-'+_id).html());
		$('#status-'+_id).val($('#sp-status-'+_id).html());
		$('#id-'+_id).show();
		$('#locationid-'+_id).show();
		$('#settlementdate-'+_id).show();
		$('#status-'+_id).show();
	...
	function saveHeader(_id, e) {
		...
		if (_confirm) {
			...
			var _settlementdate = $('#settlementdate-'+_id).val();
			var _status = $('#status-'+_id).val();

			$.post(_HOST+'App/updateSettlemethdr', {id:_id,locationid:_locationid,settlementdate:_settlementdate,status:_status}, function(result) {
				alert(result);
				location.reload();
			});
		} else {
			...
			$('#sp-settlementdate-'+_id).show();
			$('#sp-status-'+_id).show();
			$('#btn-edit-header-'+_id).show();
			
			$('#sp-id-'+_id).hide();
			$('#locationid-'+_id).hide();
			$('#settlementdate-'+_id).hide();
			$('#status-'+_id).hide();
