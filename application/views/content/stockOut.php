...
<body>
            ...
            <div class="card p-4">
                ...
                <div class="form-row">
                    <div class="form-column">
                        <label for="stockoutdate" class="form-label mt-2"><strong>DATE</strong><span class="text-danger">*</span></label>
                    ...
                    <div class="form-column">
                        ...
                        <select id="stockmovement" name="stockmovement" class="" required="true" aria-required="true">
                            <option value="">-NONE-</option>
                            <option value="1">Movement (Out)</option>
                            <option value="2">Refund (Out)</option>
                        </select>
                    ...
                    <div class="form-column">
                        <label for="toLocationId" class="form-label mt-2"><strong>TO WAREHOUSE:</strong></label>
                        ...
                        <label for="fromLocationId" class="form-label mt-2"><strong>FROM WAREHOUSE:</strong></label>
                       <?php if ($level == 1) { ?>
                            <!-- Jika level = 1 -->
                            <select id="fromLocationId" name="fromLocationId" class="" required="true" aria-required="true">
                                <option value="">-NONE-</option>
                               <!-- <?php foreach ($locationListEmployee as $j) { ?>
                                    <?php if ($j['id'] == $location_id && $j['warehouse_name'] !== null) { ?>
                                        <option value="<?= $j['id'] ?>" <?= (isset($input['fromLocationId']) && $input['fromLocationId'] == $j['id'] ? 'selected' : '') ?>>
                                            <?=$j['warehouse_name']?>
                                        </option>
                                    <?php } ?>
                                <?php } ?> -->
                        ...
                        <?php } else { ?>
                            <!-- Jika level ≠ 1 -->
                            <?php $allowedIds = array_column($locationList, 'id'); ?>
                            <select id="fromLocationId" name="fromLocationId" class="" required="true" aria-required="true">
                                <option value="">-NONE-</option>
                                <?php foreach ($locationListEmployee as $j) { ?>
                                    <?php if (in_array($j['id'], $allowedIds) && !empty($j['warehouse_name'])) { ?>
                                        <option value="<?= $j['id'] ?>" <?= (isset($input['fromLocationId']) && $input['fromLocationId'] == $j['id'] ? 'selected' : '') ?>>
                                            <?= $j['warehouse_name'] ?>
                                        </option>
                                    <?php } ?>
    ...
    <script>
        ...
        function saveStockOut(status) {
            ...
            $.ajax({
                url: "<?= base_url() . 'ControllerPOS/saveStockOut' ?>",
                type: 'POST',
                ...
                success: function(response) {
                    try {
                        ...
                        alert('berhasil menambahkan stock out');
