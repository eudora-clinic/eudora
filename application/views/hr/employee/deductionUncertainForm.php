...
<form id="uncertainAllowanceForm" onsubmit="return false;">
    <input type="hidden" name="id" value="<?= $deductionUncertain['id'] ?? '' ?>">
    <input type="hidden" name="employeeid" id="employeeid" value="<?= $deductionUncertain['employeeid'] ?? '' ?>">
    <input type="hidden" name="employeecompanyid" id="employeecompanyid"
        value="<?= $deductionUncertain['employeecompanyid'] ?? '' ?>">
    <input type="hidden" name="companyid" id="companyid" value="<?= $deductionUncertain['companyid'] ?? '' ?>">
    <input type="hidden" name="payrollid" id="payrollid" value="<?= $deductionUncertain['payrollid'] ?? '' ?>">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <select id="deductionid" name="deductionid" class="form-control" required>
                        <option value="">Select Jenis Potongan</option>
                        <?php foreach ($deductiontype as $type) { ?>
                            <option value="<?= $type['id'] ?>" <?= (isset($deductionUncertain['deductionid']) && $deductionUncertain['deductionid'] == $type['id'] ? 'selected' : '') ?>
                ...
                <div class="mb-3">
                    <label class="form-label">Jumlah Potongan <span class="text-danger">*</span></label>
                    <input type="text" name="amount" class="form-control"
                        value="<?= isset($deductionUncertain['amount']) ? number_format($deductionUncertain['amount'], 0, ',', '.') : '' ?>"
                        required oninput="formatRupiah(this)" placeholder="Masukkan jumlah potongan">
                </div>

                <div class="mb-3">
                    <label class="form-label">Deskripsi</label>
                    <textarea name="description" id="description" class="form-control" rows="4"><?= $deductionUncertain['description'] ?? '' ?></textarea>
                </div>
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="companyName" class="form-control" readonly
                        value="<?= $deductionUncertain['companyname'] ?>">
                    <small class="text-muted">Perusahaan pemberi potongan</small>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="isactive" class="form-control" required="true" aria-required="true">
                        <option value="1" <?= (isset($deductionUncertain['isactive']) && $deductionUncertain['isactive'] == 1) ? 'selected' : 'selected' ?>>Aktif</option>
                        <option value="0" <?= (isset($deductionUncertain['isactive']) && $deductionUncertain['isactive'] == 0) ? 'selected' : '' ?>>Tidak Aktif</option>
                    </select>
                </div>
            </div>
        </div>
    ...
    <div class="row mt-3">
        <div class="col-12">
            <div class="alert alert-info">
                ...
                <ul class="mb-0">
                    <li>Potongan akan ditambahkan ke payroll karyawan</li>
                    <li>Pastikan jenis potongan sesuai dengan kebijakan perusahaan</li>
                    <li>Status "Tidak Aktif" akan menonaktifkan potongan tanpa menghapus data</li>
                </ul>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        if (isEdit) {
            ...
            let employeeName = "<?= $deductionUncertain['employeename'] ?? '' ?>";
        ...
        $("#employeeSearch").select2({
            ...
            dropdownParent: $("#uncertainDeductionModal"),
            ajax: {
    ...
    function saveDeduction() {
        ...
        if (parseFloat(amountValue) === 0) {
            showFormErrors({ amount: 'Jumlah potongan tidak boleh 0' });
            return;
        }

        const saveBtn = document.querySelector('#uncertainDeductionModal .btn-primary');
        ...
        const $modal = $('#uncertainDeductionModal');


        fetch('<?= base_url('ControllerHr/saveEmployeeDeductionUncertain') ?>', {
            method: 'POST',
            body: formData
        })
